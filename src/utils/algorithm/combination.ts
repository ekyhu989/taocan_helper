...
export interface CombinationParams {
  ...
  /** 预算模式（per_capita=按人均, total_control=按总额） */
 ...
 * 
 * 评分标准：
 * - 约束检查：低于下限或高于上限直接0分淘汰
 * - 预算接近度（40分）：越接近上限分数越高（最大化福利）
 * - 品类多样性（30分）：品类越多分数越高
...
function calculateCombinationScore(
  ...
  const minPerCapita = params.minPerCapita || 0;
  const perCapitaAmount = combination.perCapitaPrice;

  // ★ 核心修复：下限约束检查
  // 低于人均标准（下限）的组合直接0分淘汰
  if (minPerCapita > 0 && perCapitaAmount < minPerCapita) {
    return 0;
  }

  // ★ 上限约束检查
  // 超过人均上限的组合严格惩罚
  const targetBudget = params.targetBudget;
  const maxPerCapita = params.budgetMode === 'per_capita' ? targetBudget : (targetBudget / params.peopleCount);
  if (perCapitaAmount > maxPerCapita * (1 + tolerance)) {
    return 0;
  }

  let score = 0;

  // 1. 预算接近度（40分满分）- 越接近上限越好
  const deviation = combination.budgetDeviation;
  
  if (deviation > 0) {
    // 超预算但在容忍范围内
    if (deviation <= tolerance) {
      score += 40 * (1 - deviation / tolerance * 0.3);
    } else {
      score += Math.max(0, 10 - deviation * 50);
    }
  } else {
    // 低于预算：越接近上限（偏差越小）分数越高
    // deviation为负数，越接近0越好
    const absDeviation = Math.abs(deviation);
    if (absDeviation <= tolerance) {
      // 在容忍范围内，接近上限给高分
      score += 35 + 5 * (1 - absDeviation / tolerance);
    } else {
      // 偏离较大，按比例递减
      score += Math.max(0, 35 * (1 - absDeviation));
    }
  }
...
function generateCandidateCombination(
  ...
  targetBudget: number,
  peopleCount: number,
  budgetMode: 'per_capita' | 'total_control' = 'per_capita',
  minPerCapita: number = 0
): SchemeItem[] {
  ...
  for (const product of shuffled) {
    ...
    if (unitPrice <= remainingBudget) {
      ...
      if (budgetMode === 'per_capita') {
        ...
        const perCapitaCost = unitPrice;
        const perCapitaLimit = targetBudget;
        
        if (perCapitaCost > perCapitaLimit && items.length >= 4) {
          continue;
        }
        ...
        quantity = Math.max(minQuantity, Math.min(suggestedQuantity, Math.floor(remainingBudget / unitPrice)));
      }
  ...
  // ★ 核心修复：下限补偿
  // 如果组合总价低于人均标准，尝试添加更多商品补足
  if (minPerCapita > 0 && budgetMode === 'per_capita') {
    const currentTotal = items.reduce((sum, item) => sum + item.subtotal, 0);
    
    if (currentTotal < minPerCapita) {
      const deficit = minPerCapita - currentTotal;
      
      // 从未选中的商品中找到能补足差额的商品
      const selectedIds = new Set(items.map(item => item.product.id));
      const unselectedProducts = shuffled.filter(p => !selectedIds.has(p.id));
      
      // 按价格升序排列，优先选低价商品补足
      const sortedUnselected = unselectedProducts.sort((a, b) => a.price - b.price);
      
      for (const product of sortedUnselected) {
        if (currentTotal + product.price > targetBudget) continue;
        if (currentTotal >= minPerCapita) break;
...
export function generateOptimalCombination(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    // 生成候选组合（传递budgetMode）
    let candidateItems = generateCandidateCombination(products, targetBudget, peopleCount, budgetMode, minPerCapita);
    ...
    // ✅ 单人份模式：totalPrice 已经是单人份小计之和（如 55+89.9+35=179.9）
    // perCapitaPrice 直接使用 totalPrice（不需要再除以 peopleCount）
    const perCapitaPrice = budgetMode === 'per_capita' 
      ? totalPrice   // 单人份模式：直接使用
  ...
  if (!bestCombination) {
    ...
    // ✅ 按照PATCH-001文档：perCapitaPrice = totalPrice / peopleCount
    const fallbackPerCapita = fallbackTotal / peopleCount;
    
    bestCombination = {
      items: fallbackItems,
      totalPrice: fallbackTotal,
      perCapitaPrice: Math.round(fallbackPerCapita * 100) / 100,
