                ...
                <div className="mt-2 p-3 bg-blue-50 rounded-lg">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-blue-600 font-medium">
                      {formData.fundSource === '工会经费' ? '工会经费 8折' : `${formData.fundSource} 9折`}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm mt-1">
                    <span className="text-gray-600">人均上限</span>
                    <span className="font-medium">
                      ¥{(perCapita / (formData.fundSource === '工会经费' ? 0.8 : 0.9)).toFixed(2)}
                    </span>
                  </div>
                  <p className="mt-1 text-xs text-gray-500">
                    通过{formData.fundSource === '工会经费' ? '工会经费8折' : `${formData.fundSource}9折`}政策，实际可采购金额上限提升至人均标准的{formData.fundSource === '工会经费' ? '1.25' : '1.11'}倍
                  </p>
                </div>
              )}
              {showWarning && (
