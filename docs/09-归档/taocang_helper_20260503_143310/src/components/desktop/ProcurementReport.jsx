...
function extractDepartment(content) {
  ...
  const match = content.match(/申请部门[：:]\s*([^\n\r]+)/);
  return match ? match[1].trim() : null;
}

/** 从报告文本中提取申请人 */
function extractApplicant(content) {
  if (!content) return null;
  const match = content.match(/申请人[：:]\s*([^\n\r]+)/);
  return match ? match[1].trim() : null;
}
...
const ProcurementReport = ({ 
  ...
  // 支持两种格式：1. 字符串格式（旧版）2. 对象格式 {title, body}（新版）
  let reportContent;
  let reportTitle;
  
  if (isExample) {
    reportContent = typeof mockData.exampleReport === 'string' 
      ? mockData.exampleReport 
      : mockData.exampleReport.body || '';
    reportTitle = '标准公文示例';
  } else if (report && typeof report === 'object') {
    // 新版：report 是对象 {title, body}
    reportContent = report.body || '';
    reportTitle = report.title || '采购申请报告';
  } else if (typeof report === 'string') {
    // 旧版：report 是字符串
    reportContent = report;
    reportTitle = '采购申请报告';
  } else {
    reportContent = mockData.purchaseReport || '';
    reportTitle = '采购申请报告';
  }
