...
/** 获取折扣信息（统一入口） */
export const getDiscountInfo = (fundingSource: string): { rate: number; label: string } => {
  switch (fundingSource) {
    case 'union':
      return { rate: 0.8, label: '工会经费 8折' };
    case 'special_consolation':
      return { rate: 0.9, label: '专项慰问费 9折' };
    case 'administrative':
      return { rate: 0.9, label: '行政福利费 9折' };
    case 'other':
      return { rate: 0.9, label: '其他经费 9折' };
    default:
      return { rate: 0.9, label: '9折' };
  }
};
